postApp.service('homeService', function ($rootScope,$http, $location,notificationService) {
    ...
    service.teacherDetailsResponse = function (access_token, callback) {
        $http({
            ...
            headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Authorization': 'Bearer ' + access_token }, ignoreLoadingBar: true,
    ...
    $http({
            method: 'GET',
            url: api_base_url+'api/students/'+studentId+'/counters/classid='+classId,
            headers: {'Authorization':'Bearer '+access_token},  ignoreLoadingBar: true,  
        }).success(function(data, status, headers, config){
            callback(data);
        }).error(function (data, status, headers, config) {
            if(status == 400){
    ...
    service.bookDemoSubmit = function (name, job_title, email, contact_no, school, postcode, date, callback) {
        $http({
            ...
            headers: { 'Content-Type': 'application/json' }, ignoreLoadingBar: true,
        }).success(function (data, status, headers, config) {
            data.status = true;
            callback(data);
        }).error(function (data, status, headers, config) {
